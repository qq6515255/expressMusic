<template lang="html">
  <div class="admin_article_comment">
    <nav-bread>
      <span><router-link to="/admin_article">内容管理</router-link></span> /
      <span>{{ articleData.title }}</span>
    </nav-bread>
    <div class="table-box">
      <div class="table-top">
        <button type="button" name="button">
          评论总数 ({{ commentDataLength }})
        </button>
      </div>
      <table>
        <tr>
...
export default {
  ...
  methods: {
    getData() {
      this.$http
        .get("/admin/admin_content_comment?_id=" + this.$route.query._id)
        .then(response => {
    ...
    del(index, item, articleData_id) {
      ...
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$http
            .post("/admin/admin_content_comment", {
              user: item.user,
